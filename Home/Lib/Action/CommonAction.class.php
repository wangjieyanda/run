...
class CommonAction extends Action {
    ...
    public function arcList($colid = 0, $key = '', $date = '', $search = '') {
        ...
        if ($date == '') {
            ...
            $list = $obj->relation(true)->order('ord asc,createtime desc')->where($where)->limit($offset, $arcnum)->select();
            if (!$list) {
                return '';
            }
            foreach ($list as $k => $v) {
            ...
            $obj = new Model();
            $list = $obj->query("SELECT a.arcid,a.content,a.colid,a.description,a.createtime,a.click,a.ord,a.title,a.keyword,a.pic,b.colname FROM `" . C('DB_PREFIX') . "article` AS a LEFT JOIN `" . C('DB_PREFIX') . "columns` AS b ON a.colid=b.colid WHERE FROM_UNIXTIME(a.createtime,'%Y-%m')='" . $date . "' AND a.ischeck=1 ORDER BY a.ord ASC,a.createtime DESC LIMIT " . $offset . ",10");
            if (!$list) {
                return '';
            }
            foreach ($list as $k => $v) {
